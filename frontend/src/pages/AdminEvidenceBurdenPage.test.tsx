...
const renderPage = () => render(<MemoryRouter initialEntries={['/admin/sessions/ses_demo/evidence-burden']}><AdminAuthProvider><Routes><Route path="/admin/sessions/:sessionId/evidence-burden" element={<AdminEvidenceBurdenPage />} /></Routes></AdminAuthProvider></MemoryRouter>)
const enterKey = () => {
...
describe('AdminEvidenceBurdenPage', () => {
  ...
  it('requires an in-memory administrator key before requesting metrics', () => {
    renderPage()

    expect(screen.getByLabelText('관리자 Demo API Key')).toHaveAttribute('type', 'password')
    expect(adminEvidenceBurdenProvider.get).not.toHaveBeenCalled()
  })

  it('shows policy-neutral server metrics without exposing the key', async () => {
    renderPage()
    enterKey()

    await waitFor(() => expect(adminEvidenceBurdenProvider.get).toHaveBeenCalledWith('demo-secret', 'ses_demo', expect.any(AbortSignal)))
