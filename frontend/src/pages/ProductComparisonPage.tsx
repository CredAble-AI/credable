...
function ProductCard({ item }: { item: ProductView }) {
  ...
  return <article className={`product-card product-card--${(product.conditionStatus ?? 'PUBLIC_ONLY').toLowerCase()}`}>
    ...
    <section aria-labelledby={`${product.productId}-public`}><h3 id={`${product.productId}-public`}>은행 공개 상품 조건</h3><dl><div><dt>공개 최대 한도</dt><dd>{formatMoney(product.publicConditions.maxAmount)}</dd></div><div><dt>공개 금리 범위</dt><dd>{formatRate(product.publicConditions.annualRateRange)}</dd></div><div><dt>기간</dt><dd>{formatTerm(product.publicConditions.termRangeMonths)}</dd></div><div><dt>상환방식</dt><dd>{product.publicConditions.repaymentMethods.join(' · ') || '확인되지 않음'}</dd></div></dl></section>
    <section className="personalized-panel" aria-labelledby={`${product.productId}-personal`}><h3 id={`${product.productId}-personal`}>개인화 조회 조건</h3>{personalized && personalizedConditions ? <dl><div><dt>조회 한도</dt><dd>{formatMoney(personalizedConditions.maxAmount)}</dd></div><div><dt>조회 금리</dt><dd>{formatRate(personalizedConditions.annualRateRange)}</dd></div><div><dt>조회 기간</dt><dd>{formatTerm(personalizedConditions.termRangeMonths)}</dd></div></dl> : <p className="condition-note">{status.note}</p>}</section>
