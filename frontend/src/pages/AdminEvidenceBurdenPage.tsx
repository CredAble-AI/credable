...
function AdminEvidenceBurdenPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const response = await adminEvidenceBurdenProvider.get(apiKey, sessionId, controller.signal)
  ...
  const resetKey = () => { clear(); setResult(null); setError(null) }
  ...
  return <main id="main-content" tabIndex={-1} className="admin-main"><div className="admin-container admin-burden"><nav className="admin-burden-back" aria-label="세션 관리자 화면"><Link to="/admin/reviews">← 검토 목록</Link><Link to={`/admin/sessions/${encodeURIComponent(sessionId)}/audit`}>세션 처리 이력</Link></nav><header className="admin-heading"><p>EVIDENCE BURDEN METRICS</p><h1>Evidence 요청 부담 지표</h1><span>서버가 교차검증한 Evidence 요청·제출·품질검증·후속 처리 횟수를 확인합니다.</span></header>
    {!apiKey ? <AdminApiKeyForm /> : <><section className="admin-burden-context" aria-label="조회 중인 세션"><div><span>SESSION ID</span><code>{sessionId}</code></div><button type="button" onClick={() => void load()} disabled={loading}>상태 새로고침</button></section><div className="admin-live" role="status" aria-live="polite">{loading ? 'Evidence 부담 지표를 불러오고 있습니다.' : error ? 'Evidence 부담 지표를 확인하지 못했습니다.' : result ? '서버가 집계한 Evidence 부담 지표를 표시합니다.' : 'Evidence 부담 지표를 확인해주세요.'}</div>
