...
function EvidenceSelectionPage() {
  ...
  return <div className="workspace-shell customer-flow"><Header /><main id="main-content" tabIndex={-1} className="assessment-page evidence-page"><div className="container assessment-page__inner">
    ...
    <section className="assessment-actions"><div><strong>{selection?.status === 'SELECTED' ? '선택된 자료를 제출해주세요' : '추가 자료 확인 상태를 확인해주세요'}</strong><p>{selection?.status === 'SELECTED' ? '요청 자료를 다운로드하여 제출하면 서버가 파일의 출처와 품질을 실제로 확인합니다.' : '자동 확인이 중단된 경우 임의의 자료를 요구하지 않습니다.'}</p></div><div><Link className="button button--secondary" to="/assessment">기준평가로 돌아가기</Link></div></section>
  </div></main></div>
}
