...
const statusCopy: Record<ProductConditionStatus, { label: string; icon: string; message: string }> = {
  ...
  POLICY_NOT_CONFIGURED: { label: '개인화 정책 확인 불가', icon: '!', message: '현재 이 상품의 개인화 조회 정책을 확인할 수 없습니다.' },
...
const dateTime = (value: string | null) => value ? new Intl.DateTimeFormat('ko-KR', { dateStyle: 'long', timeStyle: 'short' }).format(new Date(value)) : '확인되지 않음'
...
function DetailContent({ item, onRetry, busy }: { item: ProductView; onRetry: () => void; busy: boolean }) {
  ...
  return <>
    <header className="detail-heading"><Link to="/products" className="detail-back">← 상품 비교로 돌아가기</Link><div className="detail-heading__badges"><span className={`detail-status detail-status--${(product.conditionStatus ?? 'PUBLIC_ONLY').toLowerCase()}`}><b aria-hidden="true">{status.icon}</b>{status.label}</span></div><h1>{product.productName}</h1><p>고객이 선택한 상품의 현재 확인 가능한 조건입니다. 최종 조건은 은행의 정식 심사와 약정 과정에서 확정됩니다.</p></header>
    ...
    <div className="detail-grid">
      ...
      <section className="detail-card"><p className="detail-card__number">02</p><h2>금리 정보</h2><dl><div><dt>은행 공개 금리 범위</dt><dd aria-label={`은행 공개 금리 범위 ${rate(product.publicConditions.annualRateRange)}`}>{rate(product.publicConditions.annualRateRange)}</dd></div><div><dt>개인화 조회 금리</dt><dd aria-label={`개인화 조회 금리 ${rate(personalizedConditions?.annualRateRange ?? null)}`}>{personalized ? rate(personalizedConditions?.annualRateRange ?? null) : '산출되지 않음'}</dd></div><div><dt>공개 조건 기준일</dt><dd>{product.officialSource.effectiveDate}</dd></div><div><dt>개인화 결과 시점</dt><dd>{dateTime(personalizedConditions?.queriedAt ?? null)}</dd></div></dl></section>
      <section className="detail-card"><p className="detail-card__number">03</p><h2>한도 정보</h2><dl><div><dt>은행 공개 최대 한도</dt><dd aria-label={`은행 공개 최대 한도 ${money(product.publicConditions.maxAmount)}`}>{money(product.publicConditions.maxAmount)}</dd></div><div><dt>개인화 조회 한도</dt><dd aria-label={`개인화 조회 한도 ${money(personalizedConditions?.maxAmount ?? null)}`}>{personalized ? money(personalizedConditions?.maxAmount ?? null) : '산출되지 않음'}</dd></div><div><dt>산출 불가 안내</dt><dd>{personalized ? '해당 없음' : status.message}</dd></div></dl></section>
      <section className="detail-card"><p className="detail-card__number">04</p><h2>이용 조건</h2><dl><div><dt>은행 공개 기간</dt><dd>{term(product.publicConditions.termRangeMonths)}</dd></div><div><dt>개인화 조회 기간</dt><dd>{personalized ? term(personalizedConditions?.termRangeMonths ?? null) : '산출되지 않음'}</dd></div><div><dt>상환방식</dt><dd>{product.publicConditions.repaymentMethods.join(' · ') || '확인되지 않음'}</dd></div><div><dt>수수료·중도상환·필요 자료</dt><dd>은행 심사 후 확인</dd></div></dl></section>
    </div>
    <section className="detail-source" aria-labelledby="detail-source-title"><div><h2 id="detail-source-title">출처와 기준 정보</h2><p>공개 조건과 개인화 조회 결과의 기준을 확인해주세요.</p></div><dl><div><dt>출처</dt><dd>{product.officialSource.sourceName}</dd></div><div><dt>출처 기준일</dt><dd>{product.officialSource.effectiveDate}</dd></div><div><dt>최종 은행 확인</dt><dd>{product.finalApprovalRequired ? '필요' : '확인되지 않음'}</dd></div></dl><CustomerTechnicalDetails><dl><div><dt>상품 버전</dt><dd><code>{product.productVersion}</code></dd></div><div><dt>정책 버전</dt><dd><code>{personalizedConditions?.policyVersion ?? '제공되지 않음'}</code></dd></div>{product.conditionReasonCode && <div><dt>상태 코드</dt><dd><code>{product.conditionReasonCode}</code></dd></div>}</dl></CustomerTechnicalDetails></section>
