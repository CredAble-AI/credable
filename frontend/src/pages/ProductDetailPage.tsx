...
function ProductDetailPage() {
  ...
  return <div className="workspace-shell customer-flow"><Header /><main id="main-content" tabIndex={-1} className="product-detail-page"><div className="container product-detail-page__inner"><CustomerFlowSteps current="products" className="detail-steps" /><div className="detail-live" role="status" aria-live="polite">{loading ? '선택한 상품의 현재 조건을 확인하고 있습니다.' : error ? '상품 상세를 확인하지 못했습니다.' : product ? '상품 상세 조건을 확인했습니다.' : '현재 세션에서 상품을 찾을 수 없습니다.'}</div>{error && <section className="detail-error" role="alert"><div><strong>{error.message}</strong><small>오류 코드: {error.code}{error.requestId ? ` · Request ID: ${error.requestId}` : ''}</small></div>{error.retryable && <button type="button" onClick={() => void load()}>다시 확인</button>}</section>}{loading && !product && <div className="detail-skeleton" aria-hidden="true"><span /><span /></div>}{!loading && !error && !product && <section className="detail-empty"><h1>상품을 찾을 수 없습니다</h1><p>현재 세션에 포함되지 않은 상품이거나 잘못된 상품 ID입니다.</p><Link className="button button--primary" to="/products">상품 비교로 돌아가기</Link></section>}{product && <DetailContent item={product} onRetry={() => void load(true)} busy={loading} />}</div></main></div>
}
