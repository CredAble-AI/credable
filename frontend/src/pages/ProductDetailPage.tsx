...
function DetailContent({ item, onRetry, busy }: { item: ProductView; onRetry: () => void; busy: boolean }) {
  ...
  const status = statusCopy[condition.status]
  const personalized = condition.status === 'PERSONALIZED_AVAILABLE'
  return <>
    <header className="detail-heading"><Link to="/products" className="detail-back">← 상품 비교로 돌아가기</Link><div className="detail-heading__badges"><span className={`detail-status detail-status--${condition.status.toLowerCase()}`}><b aria-hidden="true">{status.icon}</b>{status.label}</span><span>Mock · Demo Only</span></div><h1>{product.productName}</h1><p>고객이 선택한 상품의 현재 확인 가능한 조건입니다. 최종 조건은 은행의 정식 심사와 약정 과정에서 확정됩니다.</p></header>
    <section className="detail-notice" aria-labelledby="detail-status-title"><span aria-hidden="true">{status.icon}</span><div><h2 id="detail-status-title">현재 상품 결과 상태</h2><p>{status.message}</p>{condition.reasonCode && <small>상태 코드: {condition.reasonCode}</small>}{condition.status === 'QUERY_FAILED' && <button type="button" onClick={onRetry} disabled={busy}>{busy ? '다시 조회 중…' : '상품 조건 다시 조회'}</button>}</div></section>
    <div className="detail-grid">
      ...
      <section className="detail-card"><p className="detail-card__number">02</p><h2>금리 정보</h2><dl><div><dt>은행 공개 금리 범위</dt><dd aria-label={`은행 공개 금리 범위 ${rate(product.annualRateRange)}`}>{rate(product.annualRateRange)}</dd></div><div><dt>개인화 조회 금리</dt><dd aria-label={`개인화 조회 금리 ${rate(condition.personalizedAnnualRateRange)}`}>{personalized ? rate(condition.personalizedAnnualRateRange) : '산출되지 않음'}</dd></div><div><dt>공개 조건 기준일</dt><dd>{product.officialSource.effectiveDate}</dd></div><div><dt>개인화 결과 시점</dt><dd>{dateTime(condition.queriedAt)}</dd></div></dl></section>
      <section className="detail-card"><p className="detail-card__number">03</p><h2>한도 정보</h2><dl><div><dt>은행 공개 최대 한도</dt><dd aria-label={`은행 공개 최대 한도 ${money(product.publicMaxAmount)}`}>{money(product.publicMaxAmount)}</dd></div><div><dt>개인화 조회 한도</dt><dd aria-label={`개인화 조회 한도 ${money(condition.personalizedMaxAmount)}`}>{personalized ? money(condition.personalizedMaxAmount) : '산출되지 않음'}</dd></div><div><dt>산출 불가 사유</dt><dd>{personalized ? '해당 없음' : (condition.reasonCode ?? '은행 심사 후 확인')}</dd></div></dl></section>
      <section className="detail-card"><p className="detail-card__number">04</p><h2>이용 조건</h2><dl><div><dt>은행 공개 기간</dt><dd>{term(product.termRangeMonths)}</dd></div><div><dt>개인화 조회 기간</dt><dd>{personalized ? term(condition.personalizedTermRangeMonths) : '산출되지 않음'}</dd></div><div><dt>상환방식</dt><dd>{product.repaymentMethods.join(' · ') || '확인되지 않음'}</dd></div><div><dt>수수료·중도상환·필요 자료</dt><dd>은행 심사 후 확인</dd></div></dl></section>
    </div>
    <section className="detail-source" aria-labelledby="detail-source-title"><div><h2 id="detail-source-title">출처와 기준 정보</h2><p>공개 조건과 개인화 조회 결과의 기준을 확인해주세요.</p></div><dl><div><dt>출처</dt><dd>{product.officialSource.sourceName}</dd></div><div><dt>출처 기준일</dt><dd>{product.officialSource.effectiveDate}</dd></div><div><dt>정책 버전</dt><dd>{condition.policyVersion ?? '확인되지 않음'}</dd></div><div><dt>최종 은행 확인</dt><dd>{condition.finalApprovalRequired ? '필요' : '확인되지 않음'}</dd></div></dl></section>
