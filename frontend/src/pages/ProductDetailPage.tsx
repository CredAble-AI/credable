...
function DetailContent({ item, onRetry, busy }: { item: ProductView; onRetry: () => void; busy: boolean }) {
  ...
  return <>
    ...
    <div className="detail-grid">
      ...
      <section className="detail-card"><p className="detail-card__number">03</p><h2>한도 정보</h2><dl><div><dt>은행 공개 최대 한도</dt><dd aria-label={`은행 공개 최대 한도 ${money(product.publicConditions.maxAmount)}`}>{money(product.publicConditions.maxAmount)}</dd></div><div><dt>개인화 조회 한도</dt><dd aria-label={`개인화 조회 한도 ${money(personalizedConditions?.maxAmount ?? null)}`}>{personalized ? money(personalizedConditions?.maxAmount ?? null) : '산출되지 않음'}</dd></div><div><dt>산출 불가 사유</dt><dd>{personalized ? '해당 없음' : (product.conditionReasonCode ?? '은행 심사 후 확인')}</dd></div></dl></section>
    ...
    <section className="detail-source" aria-labelledby="detail-source-title"><div><h2 id="detail-source-title">출처와 기준 정보</h2><p>공개 조건과 개인화 조회 결과의 기준을 확인해주세요.</p></div><dl><div><dt>출처</dt><dd>{product.officialSource.sourceName}</dd></div><div><dt>출처 기준일</dt><dd>{product.officialSource.effectiveDate}</dd></div><div><dt>정책 버전</dt><dd>{personalizedConditions?.policyVersion ?? '확인되지 않음'}</dd></div><div><dt>최종 은행 확인</dt><dd>{product.finalApprovalRequired ? '필요' : '확인되지 않음'}</dd></div></dl></section>
