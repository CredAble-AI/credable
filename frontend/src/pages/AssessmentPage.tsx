...
function AssessmentPage() {
  ...
  return <div className="workspace-shell customer-flow"><Header /><main className="assessment-page"><div className="container assessment-page__inner">
    ...
    <section className="assessment-actions"><div><strong>{boundary ? boundaryCopy[boundary.decision.status].label : state?.status === 'COMPLETED' ? '정책 경계 상태를 확인해주세요' : '기준평가 상태를 먼저 확인해주세요'}</strong><p>{actionCopy}</p>{boundary?.decision.status === 'AMBIGUOUS' && <small>Evidence 화면 연결은 다음 기능에서 추가됩니다.</small>}{boundary?.decision.status === 'STABLE' && <small>다음 결과 화면은 후속 흐름이 확정되면 연결합니다.</small>}</div><div><Link className="button button--secondary" to="/data-connection">데이터 연결 상태 확인</Link>{state?.status === 'NOT_RUN' && <button className="button button--primary" type="button" onClick={() => void runAssessment()} disabled={phase !== 'idle'}>기준평가 실행</button>}{state && state.status !== 'NOT_RUN' && state.status !== 'COMPLETED' && <button className="button button--secondary" type="button" onClick={() => void runAssessment()} disabled={phase !== 'idle'}>기준평가 다시 실행</button>}{state?.status === 'COMPLETED' && !boundary && <button className="button button--primary" type="button" onClick={() => void checkBoundary()} disabled={phase !== 'idle'}>정책 경계 확인</button>}</div></section>
  </div></main></div>
}
