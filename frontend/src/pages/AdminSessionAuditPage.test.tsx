...
const renderPage = () => render(<MemoryRouter initialEntries={['/admin/sessions/ses_demo/audit']}><Routes><Route path="/admin/sessions/:sessionId/audit" element={<AdminSessionAuditPage />} /></Routes></MemoryRouter>)

describe('AdminSessionAuditPage', () => {
  beforeEach(() => {
    vi.mocked(adminAuditProvider.list).mockReset().mockImplementation(async (_sessionId, _limit, cursor) => cursor ? secondPage : firstPage)
  })

  it('loads audit data without an administrator credential', async () => {
    renderPage()

    await waitFor(() => expect(adminAuditProvider.list).toHaveBeenCalledWith('ses_demo', 20, null, expect.any(AbortSignal)))
    expect(screen.getByText(/합성 Demo 데이터 전용 화면/)).toBeInTheDocument()
  })

  it('renders server audit summaries and traceability', async () => {
    renderPage()
  ...
  it('uses the opaque server cursor to move between audit pages', async () => {
    ...
    await waitFor(() => expect(adminAuditProvider.list).toHaveBeenLastCalledWith('ses_demo', 20, 'cursor-next', expect.any(AbortSignal)))
    expect(await screen.findByRole('heading', { name: '세션 생성' })).toBeInTheDocument()

    fireEvent.click(screen.getByRole('button', { name: '최신 이력 보기' }))
    await waitFor(() => expect(adminAuditProvider.list).toHaveBeenLastCalledWith('ses_demo', 20, null, expect.any(AbortSignal)))
  })
