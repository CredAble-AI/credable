...
describe('liveAdminEvidenceBurdenProvider', () => {
  it('loads metrics without authentication headers', async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => response })
    vi.stubGlobal('fetch', fetchMock)
    const signal = new AbortController().signal

    await liveAdminEvidenceBurdenProvider.get('ses/demo', signal)

    expect(fetchMock).toHaveBeenCalledWith('/v1/admin/sessions/ses%2Fdemo/evidence-burden', { method: 'GET', signal })
  })
  ...
  it('preserves structured lineage errors', async () => {
    ...
    await expect(liveAdminEvidenceBurdenProvider.get('ses_demo', new AbortController().signal)).rejects.toEqual(error)
  })
})
