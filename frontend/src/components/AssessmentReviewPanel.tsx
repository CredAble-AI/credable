...
const statusCopy: Record<AssessmentReviewProcessingStatus, { title: string; description: string }> = {
  PENDING: { title: '심사역 재확인 요청이 접수됐습니다', description: '요청이 대기열에 등록됐습니다. 기존 평가 결과는 심사역 처리 전까지 그대로 유지됩니다.' },
  IN_REVIEW: { title: '심사역이 평가 결과를 확인하고 있습니다', description: '서버가 심사역 검토 시작 상태를 반환했습니다. 처리 결과가 확정될 때까지 기존 평가 결과를 유지합니다.' },
  COMPLETED: { title: '심사역 재확인이 완료됐습니다', description: '서버가 심사역 처리 결과 코드를 반환했습니다. 프론트엔드는 결과의 의미를 다시 판단하지 않습니다.' },
}
...
function AssessmentReviewPanel({ sessionId }: AssessmentReviewPanelProps) {
  ...
  if (phase === 'loading' && !result) return <section className="assessment-review assessment-review--loading" aria-label="심사역 재확인 요청 상태 확인"><span /><span /></section>
  ...
  if (!review) return <section className="assessment-review" aria-labelledby="assessment-review-title">
    ...
    {error ? error.retryable && <button className="button button--secondary" type="button" onClick={() => void send(false)}>요청 상태 다시 확인</button> : <button className="button button--primary" type="button" disabled={phase !== 'idle'} onClick={() => void send(true)}>{phase === 'requesting' ? '재확인 요청 중…' : '심사역 재확인 요청'}</button>}
  </section>

  const copy = processing ? statusCopy[processing.status] : { title: '심사역 재확인 요청이 저장됐습니다', description: '처리 상태는 서버 응답에서 확인되지 않았습니다.' }
