...
function EvidenceConsentPanel({ sessionId, selectionId, evidenceType, onConsentChanged }: EvidenceConsentPanelProps) {
  ...
  return <section className="evidence-consent" aria-labelledby="evidence-consent-title">
    ...
    <dl><div><dt>확인할 정보</dt><dd><ul className="evidence-consent__categories">{consent.dataCategories.map((category) => <li key={category}>{evidenceDataCategoryLabel(category)}</li>)}</ul></dd></div><div><dt>확인 기간</dt><dd>{consent.periodStart} ~ {consent.periodEnd}</dd></div></dl>
