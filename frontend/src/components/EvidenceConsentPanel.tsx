...
function EvidenceConsentPanel({ sessionId, selectionId, evidenceType, onConsentChanged }: EvidenceConsentPanelProps) {
  ...
  return <section className="evidence-consent" aria-labelledby="evidence-consent-title">
    <div className="evidence-consent__heading"><div><span>SELECTION-SCOPED CONSENT</span><h3 id="evidence-consent-title">이 증빙 한 건의 이용 범위</h3></div><strong className={`evidence-consent__status evidence-consent__status--${consent.status.toLowerCase()}`}>{statusLabel[consent.status]}</strong></div>
    <p>{consent.purposeDescription}</p>
    <dl><div><dt>이용 항목</dt><dd>{consent.dataCategories.join(' · ')}</dd></div><div><dt>대상 기간</dt><dd>{consent.periodStart} ~ {consent.periodEnd}</dd></div><div><dt>범위 버전</dt><dd><code>{consent.scopeVersion}</code></dd></div></dl>
    <div className="evidence-consent__action"><p className={error ? 'evidence-consent__message--error' : ''} role={error ? 'alert' : 'status'} aria-live="polite">{error?.message || message || '출처 단위 동의와 별개로 현재 선택된 증빙 범위에 동의해야 합니다.'}</p><button className={consent.status === 'GRANTED' ? 'button button--secondary' : 'button button--primary'} type="button" disabled={updating} onClick={() => void update()}>{updating ? '처리 중…' : consent.status === 'GRANTED' ? '이 동의 철회' : '이 범위에 동의'}</button></div>
  </section>
}
