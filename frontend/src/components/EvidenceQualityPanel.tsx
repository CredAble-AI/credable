...
const verifiedScopeLabels = {
  ...
  DEMO_ISSUER_IDENTITY: 'Demo 발급 서버',
} as const
...
function EvidenceQualityPanel({ sessionId, submission }: EvidenceQualityPanelProps) {
  ...
  return <section className={`evidence-quality evidence-quality--${quality.status.toLowerCase()}`} aria-labelledby="evidence-quality-title">
    ...
    {quality.trustVerification && <div className={`evidence-trust evidence-trust--${quality.trustVerification.status.toLowerCase()}`}><div><span>검증 신뢰 근거</span><strong>{quality.trustVerification.status === 'VERIFIED' ? '서버가 서명한 Demo 검증정보를 확인했습니다' : '서명된 검증정보를 확인하지 못했습니다'}</strong></div>{quality.trustVerification.status === 'VERIFIED' ? <ul>{quality.trustVerification.verifiedScopes.map((scope) => <li key={scope}>{verifiedScopeLabels[scope]}</li>)}</ul> : <p>검증할 수 없는 문서는 자동 보완평가에 사용하지 않습니다.</p>}<small>현재 확인은 CredAble 합성 Demo 발급 서버 기준이며 실제 금융기관의 발급 사실을 의미하지 않습니다.</small></div>}
