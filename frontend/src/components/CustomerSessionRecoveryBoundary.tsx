...
class CustomerSessionRecoveryBoundary extends Component<Record<string, never>, State> {
  ...
  render() {
    ...
    return <div className="workspace-shell customer-flow"><Header /><main id="main-content" tabIndex={-1} className="session-recovery-page"><section className="session-recovery-card" role="alert"><span aria-hidden="true">!</span><p className="flow-kicker">진행 복구</p><h1>진행 중인 세션을 불러오지 못했습니다</h1><p>저장된 세션은 유지했습니다. 네트워크 연결을 확인한 뒤 다시 시도해주세요.</p><small>오류 코드: {error.code}{error.requestId ? ` · Request ID: ${error.requestId}` : ''}</small><div>{error.retryable && <button className="button button--primary" type="button" onClick={this.retry}>다시 시도</button>}<Link className="button button--secondary" to="/start">새 평가 시작</Link></div></section></main></div>
  }
}
