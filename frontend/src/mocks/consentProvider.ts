...
const SCOPE_VERSION = 'demo-consent-scopes-v1'
const definitions = [
  ['BANK_INTERNAL', '은행 내부 데이터', '도입 은행이 보유한 고객·계좌·대출 관련 데이터'],
  ['CREDIT_INFORMATION', '신용정보', '은행이 정식 절차로 조회하는 신용정보'],
  ['CUSTOMER_SUBMITTED', '고객 제출 데이터', '고객이 직접 제출하는 소득·사업·재무 관련 데이터'],
  ['EXTERNAL_CONNECTED', '외부 연결 데이터', '고객 동의와 제휴 범위 안에서 연결하는 외부 데이터'],
] as const
...
const state = (sourceType: ConsentSourceType, status: ConsentStatus, timestamp: string | null = null): ConsentState => {
  ...
  const [, displayName, description] = definition
  return {
    sourceType,
    displayName,
    description,
