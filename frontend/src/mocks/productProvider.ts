...
import type { AnnualRateRange, MoneyAmount, PersonalizedProductConditions, ProductComparisonItem, ProductComparisonResult, ProductConditionStatus, ProductRequest, TermRangeMonths } from '../types/product'
...
const personalizedConditions = (maxAmount: string, minPercent: string, maxPercent: string, minMonths: number, maxMonths: number): PersonalizedProductConditions => ({
  maxAmount: money(maxAmount), annualRateRange: rate(minPercent, maxPercent), termRangeMonths: term(minMonths, maxMonths),
  policyVersion: 'demo-policy-v1', queriedAt: '2026-09-04T11:20:00+09:00',
})
...
const item = (
  ...
  personalized: PersonalizedProductConditions | null = null,
  ...
  personalizedConditions: personalized, conditionStatus, conditionReasonCode,
...
const fixture = (request: ProductRequest): ProductComparisonResult => {
  ...
  const alpha = item('demo-working-capital', '사업 운영자금 플러스', '음식업 개인사업자를 위한 합성 운전자금 상품', '50000000', rate('4.20', '8.90'), term(12, 60), ['원리금균등분할상환', '만기일시상환'],
    'PERSONALIZED_AVAILABLE', personalizedConditions('24000000', '5.10', '7.30', 12, 48))
  ...
  const steady = item('demo-steady-business', '스테디 비즈니스론', '업력 조건을 확인하는 합성 사업자 상품', '70000000', rate('4.50', '9.20'), term(12, 72), ['원리금균등분할상환'], 'INSUFFICIENT_DATA', null, 'DEMO_PRODUCT_DATA_INSUFFICIENT')
  const balance = item('demo-balance-partner', '밸런스 파트너론', '사업 운영 계획을 확인하는 합성 사업자 상품', '40000000', rate('4.60', '9.10'), term(12, 48), ['원금균등분할상환'], 'QUERY_FAILED', null, 'DEMO_PRODUCT_CONDITION_UNAVAILABLE')
  return {
    sessionId: request.sessionId,
    products: [alpha, bridge, steady, balance].map((product) => ({ product, applicationLinkAvailable: false, applicationUrl: null })),
    status: 'PARTIAL',
    sortableFields: ['PUBLIC_MAX_AMOUNT', 'PUBLIC_MIN_ANNUAL_RATE', 'PERSONALIZED_MAX_AMOUNT', 'PERSONALIZED_MIN_ANNUAL_RATE'],
