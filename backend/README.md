...
소상공인 Demo에는 기존 신청과 결정 이력이 있고, 스타트업 Demo에는 신청 이력 없이 정기
신용평가 결과만 있습니다. 따라서 기존 거절·보류 고객은 Reason Code를 보조 입력으로 사용할
수 있지만, 신규 대출 탐색 고객도 신청 이력 없이 같은 서비스 흐름에 진입할 수 있습니다.

`DEMO_GRADE_*`, `DEMO_*` Reason Code와 모든 정책·모델 버전은 합성 식별자입니다. 실제 은행의
내부점수, 승인 임계값, PD, 승인한도나 금리를 만들지 않았으며 현재 CredAble 평가 결과에도
자동 반영하지 않습니다. 실제 도입 시 은행별 코드 사전과 신청·심사 원장을 Adapter에서 이
계약으로 변환하고, 신청별 신용정보 조회 목적과 범위는 해당 은행의 동의 정책으로 관리해야
합니다.
...
재현하기 위한 원장값일 뿐 위험등급·승인여부·금리·한도를 뜻하지 않습니다. 스타트업 Demo처럼
...
실행 이력을 읽을 수 있습니다. 이번 단계는 평가 재현성을 위한 계보만 연결하며, 원천 레코드를
Feature로 변환하거나 총부채·위험도·신용등급을 새로 계산하지 않습니다.
